...
#used Claude to understand how to use yt-dlp as subprocess and how to call librosa to get features
...
def download_song(track_name: str, artist: str) -> str:
    ...
    # safe file name so OS doesn't interpret parts of name as a path
    ...
    # %(ext)s is a yt-dlp placeholder, fills in actual extension
    ...
    cmd = [
        "py", "-m", "yt_dlp",
        "--ffmpeg-location", r"C:\Users\agrec\AppData\Local\Microsoft\WinGet\Packages\Gyan.FFmpeg_Microsoft.Winget.Source_8wekyb3d8bbwe\ffmpeg-8.1-full_build\bin",
        "--extract-audio",            # strip video stream, keep audio only
        "--audio-format", AUDIO_FORMAT,  # convert to mp3 (requires ffmpeg)
    ...
    # run yt-dlp as subprocess, capture output
...
#loads audio file and extracts features for tone classifier
def extract_features(filepath: str) -> dict:

    print(f"[features] Loading audio: {filepath}")

    # 22050 is standard sample rate for audio ML, duration=60 so we don't process full clip
    ...
    # energy: using RMS to find average loudness of signal
...
#pipeline wrapper: single call that downloads, extracts, and returns information, called by app.py
...
#test to ensure pipeline works, run with python audio_features.py
